...
$paginanaam="Zorgverleners selecteren";
  ...
  if ($_GET['menos']==1) {
    ...
    $action = "overleg_plannen_select_zvl_twee.php?menos=1";
  }
  else {
    $action = "overleg_plannen_select_zvl_twee.php";
  }
  ...
  if (!empty($vanuitZorgTeam) || ($_POST['vanWaar'] == "zorgTeam")) {
    ...
    $backpage =  "overleg_plannen_select_zvl_twee.php";
    ...
    $backpage =  "overleg_plannen_select_zvl_twee.php?overleg={$controleOverleg}";
    ...
    if(isset($_POST['hvl_id'])&&($_POST['hvl_id']!="")&&($_SESSION['pat_code']!=""))
        ...
        if (mysql_num_rows($doeBestaatSql)==0)
            ...
            if ($result=mysql_query($sql1)) {
               $melding="ZVLgegevens zijn <b>succesvol ingevoegd</b>.<br>";
              ...
              /************** einde overdracht tussen zorgteams GDT en Menos *************/
                ...
                {$melding="ZVLgegevens zijn <b>niet succesvol ingevoegd</b>,<br>".$sql1;}
            //--------------------------------------------------------------
            }
        else
           {$melding="ZVL Er ontbreken gegevens hvl_id of pat_code";}
        //--------------------------------------------------------------
        }
...
if ($_SESSION['isOrganisator']==1) {
  ...
  $nietZelf = " and not (h.organisatie = {$_SESSION['organisatie']} and h.naam = \"{$_SESSION['naam']}\" and h.voornaam = \"{$_SESSION['voornaam']}\") ";
    ...
    if ($records['type']==16 || $records['type']==18) {
       $controleGGZ = "not (((org.ggz = 1) or f.id in (62,76,117))) AND ";
    }
    $query = "
...
<?
    ...
    print("<h1>Zorgverleners toevoegen aan het {$menosToevoeging}zorgteam</h1>

            <p>Selecteer uit onderstaande lijst de <b>zorgverleners(*)</b> 
    ...
    /* Toon form ZVL selecteren */ include('../forms/select_zvl.php');
...
function pasOp() {  
   ...
   if (document.zvlform.IIZvl.value=='') {
     ...
     return confirm("De nieuwe zorgverlener is nog niet opgeslagen.\nWil je toch terug gaan naar het overzicht?");
...
<form action="overleg_plannen_select_zvl_twee.php" method="post" name="nextform">
