...
<td width="15%" align="center"><input type="radio" name="katz_wassen" onClick="calculateKatz('katz_wassen',0)" value="1a" <?php print($records['katz_wassen']);$selected=($records['katz_wassen']=="1a")?"checked=\"checked\"":"";print($selected);?> /> 1</td>
...
<?php
        ...
        else
            {
                ...
                if ($result=mysql_query($katz_qry1))
                    {// Query werd succesvol uitgevoerd
                    print("
                    <script>
                    function redirect()
                        {
                        //document.location = \"overleg.php?actie=afwerken&alleenGroen&a_overleg_id=".$_SESSION['overleg_id']."\";
