...
EQ = {
    ...
    ajaxProcessor: {
        ...
        startProcess: function(pname){
            if(this.curProcess=="") this.sendRequests(pname);
        },
        ...
        doAction : function(response, feedback, pname){
            if(this.process[pname](response,feedback)) return true;
            else return false;
        }
    },
    ...
    formCheck:function(elem,l,ml,email,pref){
        ...
        var b = true;
        var elemn = elem.name;
        if(elemn != "submit") elem = elem.value.toString();
        else elem = 0;
        this.errormsg = "buzy";
        if(email==true){
    ...
    login: function(json){
        if(json.uid && json.pin){
            if(localStorage){
                ...
                return true;
            }else alert("Geen ondersteuning voor localstorage. U kan niet aanmelden!");
            return false;
        }else
            return false;
    },
    overflow:function(o){
        ...
        this.refresh = function(a){
            switch(a){
                case "head": case "h": case"header":
                    if(this.attached) this.head= this.attached.h; 
                    else this.head = "<hgroup><h3>"+this.title+"</h3><h7>"+this.sub+"</h7></hgroup>";
                    $("#vrflheader").html(this.head);
                    break;
                default: case "content": case "c":
                    if(this.attached) this.content= this.attached.c; 
                    $("#vrflcontent").html(this.content);
                    break;
                case "footer": case "f":
                    $("#overflowcontent footer").html(this.footer + this.button);
        ...
        var xtra = extra || "";
        switch(url){
            case "home": url = "/listel_new/ecareplan/home/welkom/"; break;
            case "login": url = "/listel_new/ecareplan/login/login/"; break;
            case "eid": url="/listel_new/ecareplan/login/eid/"; break;
            
            case "omb": url="/listel_new/ecareplan/omb/"; break;
            case "ombbewerk": url="/listel_new/ecareplan/omb/bewerk/"; break;
            case "ombnieuw": url ="/listel_new/ecareplan/omb/nieuw/"; break;
            
            case "overleg": url="/listel_new/ecareplan/overleg/toon/"; break;
            case "overlegbewerk": url="/listel_new/ecareplan/overleg/bewerk/"; break;
            case "overlegnieuw": url="/listel_new/ecareplan/overleg/nieuw/"; break;
            default: break;
        }
        url+=xtra+EQ.getUid();
